...
// OpenAI chat-completions SSE responses (the wire shape the
// Convergent Systems gateway serves — see core-infra). Internals
// (rpc dispatcher, csllm client, reliab) are exercised through the
// wire — this suite is the contract pin for the cmd/ wiring, not
// for the packages under internal/.
//
// What this suite proves:
//
//   - --version exits 0 with the expected prefix.
//   - Missing $CS_API_KEY (or legacy $ANTHROPIC_API_KEY) produces a
//     non-zero exit with a diagnostic that names the env var but
...
// CS_API_KEY in subprocess env and to assert it never leaks
// through any user-visible output path.
package integration
